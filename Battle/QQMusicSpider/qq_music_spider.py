...
def download(guid, songmid,cookie_dict,songname,albumname):
    ...
    if purl:
        ...
        realMusic = r.content
        time.sleep(5)
        saveMusic(songname,albumname,realMusic)
        return True
    else:
        return False

def saveMusic(songname,albumname,realMusic):
    folder = 'E:\\PythonTest\\DownloadSong'
    albumname = re.sub(r'\W+', '_', albumname)
    songname = re.sub(r'\W+', '_', songname)

    albumPath = 'E:\\PythonTest\\DownloadSong\\' + albumname
    song = albumPath + "\\"  + songname + '.m4a'
    print(folder)
    print(albumPath)
    print(song)
    ...
    if os.path.exists(song) == False:
        f = open(albumPath + "\\" + songname + '.m4a', 'wb')
        f.write(realMusic)
        f.close()
...
def get_singer_songs(singermid,cookie_dict):
    ...
    for p in range(pagecount):
        ...
        for i in music_data:
            ...
            song_dict['song_albummid'] = i['musicData']['albummid']
            # song_dict['song_interval'] = i['musicData']['interval']
            song_dict['song_interval']=str(i['musicData']['interval']//60)+'m'+str(i['musicData']['interval']%60)+'s'
            song_dict['song_songmid'] = i['musicData']['songmid']
            song_dict['song_singer'] = song_singer
            song_dict['song_singermid'] = i['musicData']['singer'][0]['mid']

            info = download(cookie_dict['pgv_pvid'],song_dict['song_songmid'],cookie_dict,song_dict['song_name'],song_dict['song_album'])
